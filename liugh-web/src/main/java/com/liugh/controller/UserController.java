...
public class UserController {
    ...
    public ResponseModel<IPage<User>> findList(@RequestParam(name = "pageIndex", defaultValue = "1", required = false) Integer pageIndex,
                                               @RequestParam(name = "pageSize", defaultValue = "10", required = false) Integer pageSize,
                                               @RequestParam(value = "username", defaultValue = "", required = false) String username) {
        return ResponseHelper.buildResponseModel(userService.page(new Page<>(pageIndex, pageSize),
                ComUtil.isEmpty(username) ? new QueryWrapper<User>() : new QueryWrapper<User>().like("user_name",
                        username)));
    }
